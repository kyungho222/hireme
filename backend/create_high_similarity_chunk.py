...
def create_high_similarity_chunk():
    mongo = MongoService()
    ...
    # 거의 동일한 내용의 자기소개서 생성 (약간의 단어만 변경)
    high_similarity_text = """
지원동기
안녕하세요. 프론트엔드 개발자 김개발입니다. 귀사의 혁신적인 웹 서비스와 사용자 중심의 개발 문화에 매료되어 지원하게 되었습니다.

성장배경
컴퓨터공학을 전공하며 웹 개발에 대한 깊은 관심을 키워왔습니다. 대학 시절부터 다양한 프로젝트를 통해 실무 경험을 쌓았고, 특히 사용자 경험을 중시하는 프론트엔드 개발에 매료되어 이 분야로 진로를 정했습니다.
...
귀사에서 모던 웹 기술을 활용한 혁신적인 프로젝트에 참여하고 싶습니다. 사용자 경험을 최우선으로 하는 개발 문화에서 더욱 성장하여, 사용자들에게 가치 있는 서비스를 제공하는 개발자가 되고 싶습니다.
    ...
    new_cover_letter_doc = {
        ...
        'extracted_text': high_similarity_text,
    ...
    print(f'높은 유사도 자기소개서 생성 완료: {new_cover_letter_id}')

    # 높은 유사도 청크 생성 (원본과 거의 동일한 내용)
    chunk_text = high_similarity_text[:500]
    high_similarity_chunk_doc = {
        'applicant_id': str(applicant['_id']),
        ...
        'metadata': {
            ...
            'document_title': f"{applicant['name']}의 높은 유사도 자기소개서"
        },
        'created_at': datetime.datetime.now()
    }

    # 높은 유사도 청크 저장
    high_similarity_chunk_result = mongo.sync_db.cover_letter_chunks.insert_one(high_similarity_chunk_doc)
    print(f'높은 유사도 청크 생성 완료: {high_similarity_chunk_result.inserted_id}')
