...
class AutoTokenMonitor:
    ...
    def __init__(self, token_monitor: Optional[TokenMonitor] = None, config: Optional[TokenMonitorConfig] = None):
        self.token_monitor = token_monitor or TokenMonitor(config)
        self.config = config or TokenMonitorConfig()

        self.is_running = False
        self.monitor_thread = None
        self.report_interval = self.config.monitor_interval
        self.auto_report = self.config.auto_report
        self.report_threshold = self.config.report_threshold
    ...
    def _check_usage_and_alert(self):
        """사용량 확인 및 알림"""
        try:
            summary = self.token_monitor.get_usage_summary()
            daily = summary["daily"]
    ...
    def _print_console_alert(self, daily: dict, monthly: dict, status: str):
        ...
        print(f"📅 오늘: {daily.get('total_tokens', 0):,} / {self.token_monitor.limits.daily_limit:,} 토큰 ({daily.get('limit_usage_percent', 0):.1f}%)")
        print(f"📆 이번 달: {monthly.get('total_tokens', 0):,} / {self.token_monitor.limits.monthly_limit:,} 토큰 ({monthly.get('limit_usage_percent', 0):.1f}%)")
        ...
        try:
            ...
            payload = {
                ...
                "attachments": [{
                    ...
                    "fields": [
                        {"title": "오늘 사용량", "value": f"{daily.get('total_tokens', 0):,} / {self.token_monitor.limits.daily_limit:,} 토큰 ({daily.get('limit_usage_percent', 0):.1f}%)", "short": True},
                        {"title": "월간 사용량", "value": f"{monthly.get('total_tokens', 0):,} / {self.token_monitor.limits.monthly_limit:,} 토큰 ({monthly.get('limit_usage_percent', 0):.1f}%)", "short": True},
    ...
    def _log_alert(self, summary: dict):
        """알림 로그 파일 저장"""
        try:
            log_dir = self.token_monitor.data_dir / "alerts"
            ...
            log_entry = f"{datetime.now().isoformat()} - {status} - Daily: {daily.get('total_tokens', 0)}/{self.token_monitor.limits.daily_limit} ({daily.get('limit_usage_percent', 0):.1f}%) - Monthly: {monthly.get('total_tokens', 0)}/{self.token_monitor.limits.monthly_limit} ({monthly.get('limit_usage_percent', 0):.1f}%)\n"
